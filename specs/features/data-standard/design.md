...
| `data-standard.master-data` | 主数据 | `/data-standard/master-data` | 主数据匹配与权威记录工作台 |
...
| 主数据 | 从多源记录形成可信权威记录 | 实体模型导航；来源匹配队列；记录对比；权威记录裁决与分发轨迹 | 权威源、匹配置信度、关键属性冲突、保留值、分发状态 | 以多记录对比和合并裁决为中心 |
...
| 数据资产 | 一般数据资产可引用标准 ID 与版本 | 非标准资产的目录和流通 | 将数据标准发布为资产或建设申请授权流程 |
