...
| 页面任务 | 主结构 | 适用页面 |
|---|---|---|
| 控制项配置与覆盖检查 | 控制矩阵 + 规则库 + 证据检查器 | 合规清单、个人信息、分级规则、防护策略、访问控制、加密、审计证据 |
| 审批与整改推进 | 分阶段审批看板 | 合规审查、重要数据、分类复核、监控与风险、审计整改 |
| 场景判断与过程留痕 | 分步流程向导 + 材料区 | 出境评估、水印、事件 SOP、事件通知 |
| 正式成果编制 | 版本导航 + 报告编制区 + 复核门禁 | 分类报告、审计报告 |
| 计划执行与取证 | 计划日程 + 执行检查表 + 工作底稿 | 审计计划、审计执行、事件演练 |
| 事件处置 | 事件队列 + 纵向时间线 + 指挥与关闭门禁 | 事件台账 |
| 专业操作台 | 任务编排 + 结果抽检 / 规则模拟 + 执行记录 | 分类识别、脱敏 |

同一组页面可以复用交互原语，但不得仅替换标题、指标和表格列来伪装成不同功能。
