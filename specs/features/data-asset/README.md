...
数据资产运营是数据资产管理域的核心产品，统一管理数据表、数据集、文档和模型等资产，覆盖资产目录、资产流通、权属登记、价值评估、资产运营（数据产品与使用授权）和使用审计，并配套 DCMM 4 级量化管理报告，帮助组织实现从登记、申请到使用追踪的全生命周期闭环。
...
- 七个页面：资产目录 `/data-asset/catalog`、资产流通 `/data-asset/circulation`、权属登记 `/data-asset/ownership`、价值评估 `/data-asset/value`、资产运营 `/data-asset/service`、使用审计 `/data-asset/audit` 和量化报告 `/data-asset/reports`。
- 数据标准由独立 `data-standard` feature 权威维护，不作为资产目录类型，不进入资产申请、授权、对接或流通流程。
...
- 直接资产流通采用「提交申请 → 资产负责人审批 → 条件性安全审批 → 对接配置与联调 → 使用跟踪」，与数据产品使用授权分开保存。
