...
大数数据平台是面向企业的数据资产与 AI 数据服务平台, 将数据接入、数据湖、数据治理、数据开发、调度、安全和运维串成可发现、可使用、可运营的数据闭环。
...
| AI 应用开发者 · AI Builder | 通过数据服务使用可信数据 |
...
| `data-standard` | 数据标准 | ✅ 五个工作台与 SQLite mock | `/data-standard/*` |
| `data-governance` | 数据治理 | 🔨 治理中心、元数据、数据质量 SQLite 持久化 mock | `/data-governance/*` |
| `data-development` | 数据开发 | ✅ ETL/SQL/Notebook 专业工作台 P0 | `/data-development/*` |
| `data-asset` | 数据资产 | 🔨 SQLite 持久化 mock | `/data-asset/*` |
| `scheduler` | 调度引擎 | 🔨 SQLite 持久化 mock | `/scheduler/*` |
| `ops-monitor` | 运维与监控 | 🔨 SQLite 持久化 mock | `/ops-monitor/*` |
| `data-security` | 数据安全 | ✅ 25 个功能页、六分域 SQLite mock 与三级菜单 | `/data-security/*` |
| `settings` | 系统设置 | 🚧 菜单与系统配置 | `/settings/*` |
